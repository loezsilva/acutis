...
from acutis_api.communication.responses.agape import CardsEstatisticasFamiliasAgapeResponse
...
class CardsEstatisticasFamiliasAgapeUseCase:
    ...
    def execute(
        ...
        ) if total_cadastradas > 0 else 0.0
        ...
        porcent_familias_ativas = (
            total_ativas / total_cadastradas
        ) * 100 if total_cadastradas > 0 else 0.0
        porcent_familias_inativas = (
            total_inativas / total_cadastradas
        ) * 100 if total_cadastradas > 0 else 0.0
        ...
        response_data = CardsEstatisticasFamiliasAgapeResponse(
            ...
            familias_inativas=f"{total_inativas} - {porcent_familias_inativas:.0f}%"
        )
