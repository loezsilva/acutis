...
class BuscarFamiliaAgapePorCpfUseCase:
    ...
    def execute(self, cpf: str, ciclo_acao_id: UUID) -> FamiliaAgapeDetalhesPorCpfResponse:
        ...
        familia = self._agape_repository.buscar_familia_agape_por_id(membro.fk_familia_agape_id)
        if not familia: 
            # buscar_familia_agape_por_id já levanta HttpNotFoundError (incluindo soft delete)
            # Esta verificação é redundante se o método do repo sempre levanta.
