...
from acutis_api.domain.services.google_maps_service import (
    GoogleMapsAPI, GoogleMapsGeocode
)
...
class EditarEnderecoFamiliaAgapeUseCase:
    ...
    def execute(
        ...
        try:
            ...
            logger.error(f"Erro de geolocalização (HttpNotFoundError) \
            para endereço '{str_endereco}': {e}"
        )
            raise HttpNotFoundError("Endereço não pôde ser \
                geolocalizado ou é inválido."
            )
        except Exception as e:
            logger.error(f"Erro inesperado ao geolocalizar endereço \
                '{str_endereco}': {e}", exc_info=True
            )
            raise HttpNotFoundError(
                f"Erro ao geolocalizar endereço: {str(e)}"
            )
