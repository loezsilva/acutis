...
def test_buscar_membros_mes_sucesso(
    client: FlaskClient,
    membro_token,
    ...
    response = client.get(
        '/api/admin/membros/buscar-membros-mes',
        headers={'Authorization': f'Bearer {membro_token}'},
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json['membros_mes'] == 9
    assert response.json['porcentagem_crescimento'] == 800


def test_buscar_membros_mes_janeiro(
    client: FlaskClient, membro_token, seed_registrar_membro
):
    with freeze_time('2025-01-01'):
        hoje = datetime.now()
        seed_registrar_membro(criado_em=hoje)
        ...
        assert response.status_code == HTTPStatus.OK
        assert response.json['membros_mes'] == 1
        assert response.json['porcentagem_crescimento'] == 0


@patch.object(BuscarMembrosMesUseCase, 'execute')
